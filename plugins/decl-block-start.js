  ...
  if (parent) {
    ...
    for (let i = 0; i < idx; i++) {
      if (parent.body[i].type == 'VariableDeclaration')
        continue
      context.report({ node, messageId: 'declBlockStart' })
      return
    }
  }
}
...
(context) {
  return { VariableDeclaration: node => VariableDeclaration(context, node),
           FunctionDeclaration: node => FunctionDeclaration(context, node) }
}

export
default { meta: { type: 'suggestion',
                  docs: { description: 'Require variable declarations to be at the start of the block.' },
                  messages: { declBlockStart: 'Variable declarations must be at the start of the block.' },
                  schema: [] },
          create }
